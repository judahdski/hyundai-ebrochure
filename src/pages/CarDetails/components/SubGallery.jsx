...
import {
  ioniq6InteriorOverview,
  ioniq6ExteriorOverview,
} from '../../../assets/img/temp/index';
...
const SubGallery = ({ carID }) => {
  ...
  //#region Get data
  const carDetail = () =>
    carDetails.filter((row) => row.id == carID)[0].carDetail;
    ...
    <div className='p-4 md:p-[72px] pt-6 md:pt-[48px] flex flex-col gap-[36px] md:gap-[72px]'>
      ...
      <div className='flex flex-col items-center gap-2 md:gap-[32px]'>
        ...
        <div className='w-full flex flex-col md:flex-row gap-4 md:gap-[80px]'>
          <div className='flex flex-col gap-2 md:gap-4'>
            <p className='text-base md:text-2xl text-[#7C7C7C] font-medium'>
              Warna
            </p>
      ...
      <div className='flex flex-col gap-4 md:gap-8'>
        {articles.map(({ title, body, images }) => (
          <>
            <h3 className='text-xl md:text-[32px] text-center font-medium'>
              {title}
            </h3>
